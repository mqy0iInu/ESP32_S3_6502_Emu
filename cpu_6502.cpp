...
volatile const uint8_t BIOS[10496] = {
...
volatile const uint8_t BIOStop[256] = {
	0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
	0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
	0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
	0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
...
static inline uint8_t READ_BIOS(uint16_t addr)
{
	uint8_t data;
...
static inline uint8_t READ_BIOStop(uint16_t addr)
{
	uint8_t data;
...
uint8_t read6502(uint16_t address) {
  ...
  if (address >= 0xC000) {
    ...
    if (BIOSaddr < 0x2900) return READ_BIOS(BIOSaddr);
    if (BIOSaddr >= 0x3F00) return READ_BIOStop(BIOSaddr - 0x3F00);
  }
...
#ifdef USE_TIMING
      clockgoal6502 -= (int32_t)READ_BYTE(ticktable + opcode);
#endif
      instructions++;
  }
}
